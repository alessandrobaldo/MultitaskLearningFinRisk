...
class SGHMCSampler(_BaseSampler):
	...
	def sample(self, nsamples=1, nchains=1, **args):
		...
		for _ in range(nchains):
			self.run_chain(nsamples)
	
	def run_chain(self, nsamples = 1, **args):
...
class LossModule(nn.Module):
	...
	def forward(self):
		...
		loss = (self.N/self.X.shape[0])*self.loss_fn(outputs, self.labels) + self.negative_log_prior(self.model.named_parameters())
		return loss
